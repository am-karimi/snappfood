...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="antialiased">
        <div class="relative flex items-top justify-center min-h-screen bg-red-100 dark:bg-red-900 sm:items-center py-4 sm:pt-0 " >
            @if (Route::has('login'))
                <div class="hidden fixed top-0 left-0 px-6 py-4 sm:block">
                    @auth
                        <a href="{{ url('/dashboard') }}" class="text-red-700 dark:text-red-500 ">
                            Dashboard
                        </a>
                    @else
                        <a href="{{ route('login') }}" class="text-red-700 dark:text-red-500 font-bold text-4xl ">Log in</a>

                        @if (Route::has('register'))
                            <a href="{{ route('register') }}" class="ml-4 text-red-700 dark:text-red-500  font-sans text-2xl">Register</a>
                        @endif
                    @endauth
                </div>
            @endif
