...
@section('body')
    ...
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 mt-5 rounded">
        ...
        <tbody>
        <form action="{{    route('foodCategories.update',$foodCategory      )   }}" method="post">
            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                @csrf
                @method('put')
                <td class="px-6 py-4">
                    <input type="text" name="name" value="{{   $foodCategory->name    }} " class="text-black" >
                </td>
                <td class="px-6 py-4">
                    <input type="text" name="slug" value="{{   $foodCategory->slug    }}" class="text-black" >
                </td>
                <td class="px-6 py-4">
